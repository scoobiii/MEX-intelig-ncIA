...
import { SnowflakeIcon, WrenchScrewdriverIcon, BoltIcon, CloudIcon, ComputerDesktopIcon, ActivityIcon, MapPinIcon, GasIcon, ChartBarIcon, WarningIcon } from '../application/components/icons';
import DashboardCard from '../DashboardCard';
import { Page } from '../application/components/Navigation';
    ...
    return (
        <div className="mt-6">
            <div className="grid grid-cols-1 lg:grid-cols-11 gap-6 items-stretch">
                    ...
                    >
                        <div className="flex flex-col justify-center h-full space-y-4 text-sm">
                            ...
                            <div className="flex justify-between items-baseline bg-gray-900/50 p-2 rounded-lg">
                                <span className="font-semibold text-orange-400">Calor Residual (Trigeração)</span>
                ...
                <div className="lg:col-span-3">
                    <DashboardCard 
                        title="Alinhamento de Refrigeração"
                        icon={<WrenchScrewdriverIcon className="w-6 h-6 text-gray-300" />} 
                        className="h-full"
                    >
                        <div className="flex flex-col h-full justify-between">
                            <div className="space-y-3">
                                <div className="bg-gray-700/50 p-3 rounded-lg cursor-pointer hover:bg-gray-700" onClick={() => setCurrentPage('Utilities')}>
                                    <div className="flex justify-between items-baseline">
                                ...
                                <div className="bg-gray-700/50 p-3 rounded-lg cursor-pointer hover:bg-gray-700" onClick={() => setCurrentPage('Data Center')}>
                                    ...
                                    <p className="text-xs text-gray-400 mt-1">{`Potencial para ${activeRackCount.toFixed(0)} / ${TOTAL_RACKS} Racks Ativos`}</p>
                                </div>
                            </div>
                        </div>
                    </DashboardCard>
            ...
            <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                >
                    <div className="flex flex-col justify-center items-center h-full text-center">
                        ...
                        <p className="text-lg text-gray-400">MW (Equivalente Elétrico)</p>
                    </div>
                </DashboardCard>
                 <DashboardCard 
                    title="Alocado para o Data Cloud"
                ...
                >
                    <div className="flex flex-col justify-center items-center h-full text-center">
                        ...
                        <p className="text-lg text-gray-400">MW de Frio</p>
                         <p className="mt-4 text-gray-300">{`Potencial para ${activeRackCount.toFixed(0)} / ${TOTAL_RACKS} Racks Ativos`}</p>
                    </div>
                </DashboardCard>
            </div>
        </div>
...
const ConventionalView: React.FC<Pick<UtilitiesProps, 'selectedPlant'>> = ({ selectedPlant }) => {
    return (
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <DashboardCard title="Informações Gerais" icon={<MapPinIcon className="w-6 h-6"/>}>
                <div className="space-y-3 text-sm">
                    <div className="flex justify-between"><span className="text-gray-400">Localização:</span> <span className="font-semibold">{selectedPlant.locationKey || 'N/A'}</span></div>
                    <div className="flex justify-between"><span className="text-gray-400">Modo de Combustível:</span> <span className="font-semibold">{selectedPlant.fuelKey}</span></div>
                    <div className="flex justify-between"><span className="text-gray-400">Tecnologia:</span> <span className="font-semibold">{selectedPlant.cycleKey || 'N/A'}</span></div>
                    <div className="flex justify-between"><span className="text-gray-400">Potência (MW):</span> <span className="font-semibold">{selectedPlant.power} MW</span></div>
                    {selectedPlant.descriptionKey && <p className="pt-2 border-t border-gray-700 text-gray-400 italic">{selectedPlant.descriptionKey}</p>}
                </div>
            </DashboardCard>
             <DashboardCard title="Performance (Base 2023)" icon={<ChartBarIcon className="w-6 h-6"/>}>
                <div className="space-y-3 text-sm">
                    <div className="flex justify-between"><span className="text-gray-400">Geração 2023:</span> <span className="font-semibold text-cyan-400">{selectedPlant.generation2023?.toLocaleString('pt-BR') || 'N/A'} GWh</span></div>
                    <div className="flex justify-between"><span className="text-gray-400">Eficiência Total Média:</span> <span className="font-semibold text-green-400">{selectedPlant.efficiency?.toFixed(1) || 'N/A'}%</span></div>
                    <div className="flex justify-between"><span className="text-gray-400">Taxa de Emissão:</span> <span className="font-semibold text-orange-400">{selectedPlant.rate || 'N/A'} tCO₂e/GWh</span></div>
                </div>
            </DashboardCard>
             <DashboardCard title="Emissões (Base 2023)" icon={<WarningIcon className="w-6 h-6"/>}>
                <div className="flex flex-col justify-center items-center h-full text-center">
...
const Utilities: React.FC<UtilitiesProps> = ({ selectedPlant, ...props }) => {
    ...
    return isTrigenerationProject ? <TrigenerationView {...props} /> : <ConventionalView selectedPlant={selectedPlant} />;
};
