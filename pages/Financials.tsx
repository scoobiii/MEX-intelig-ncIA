...
import { ChartBarIcon, BoltIcon, ArrowDownTrayIcon, ChartPieIcon, TrendingUpIcon, ComputerDesktopIcon } from '../application/components/icons';
  ...
  activeRackCount,
    ...
    return (
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <DashboardCard
                title={'Demonstração de Resultados (DRE Mensal)'}
            ...
            >
                <div className="space-y-3 mb-4">
                    ...
                    <div className="flex justify-between items-baseline p-2">
                        <span className="text-gray-400 text-sm">{'Lucro (EBITDA)'}</span>
            ...
            <DashboardCard title={'Receita de Sustentabilidade'} icon={<BoltIcon className="w-6 h-6 text-emerald-400" />}>
                 <div className="grid grid-cols-2 gap-4 text-center h-full items-center">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500">{'USD/ton'}</p>
                    </div>
                    <div className="col-span-2 bg-gray-900/50 p-2 rounded-lg">
                        <p className="text-gray-400 text-sm">{'Receita Adicional Estimada'}</p>
            ...
            <DashboardCard title={'Receita de Serviços em Nuvem'} icon={<ComputerDesktopIcon className="w-6 h-6 text-purple-400" />}>
                <div className="space-y-3 h-full flex flex-col justify-around">
                    ...
                    <div className="flex justify-between items-baseline">
                        ...
                        <span className="font-semibold text-purple-400">{formatCurrency(financialMetrics.revenueStreamData.find(r => r.name === 'Serviços em Nuvem')?.value || 0)}</span>
                    </div>
                    <div className="flex justify-between items-baseline">
                        <span className="text-gray-400">{'Custos OPEX'}</span>
                        <span className="font-semibold text-orange-400">-{formatCurrency(financialMetrics.costData.find(c => c.name === 'Data Center (OPEX)')?.value || 0)}</span>
                    </div>
                    <div className="border-t border-gray-700 my-1"></div>
                    <div className="flex justify-between items-baseline bg-gray-900/50 p-2 rounded-lg">
                        <span className="text-gray-300 font-bold">{'Lucro Líquido (Nuvem)'}</span>
                        <span className="font-bold text-green-500">{formatCurrency((financialMetrics.revenueStreamData.find(r => r.name === 'Serviços em Nuvem')?.value || 0) - (financialMetrics.costData.find(c => c.name === 'Data Center (OPEX)')?.value || 0))}</span>
                    </div>
                </div>
            </DashboardCard>
            
            <DashboardCard title={'Fontes de Receita'} icon={<BoltIcon className="w-6 h-6" />}>
            ...
            <DashboardCard title={'Estrutura de Custos Operacionais'} icon={<ChartPieIcon className="w-6 h-6" />}>
            ...
            <DashboardCard title={'Resumo Mensal (Receita x Custos x EBITDA)'} icon={<ChartBarIcon className="w-6 h-6" />} className="lg:col-span-3">
             ...
             <DashboardCard title={'Metas de Produção'} icon={<TrendingUpIcon className="w-6 h-6" />} className="lg:col-span-3">
                <div className="space-y-4 h-full flex flex-col justify-around">
                    <div>
                     ...
                     <div>
                        <div className="flex justify-between items-baseline mb-1">
                            <span className="text-sm font-medium text-gray-300">{'Projeção de ROI'}</span>
            ...
            <DashboardCard title={'Receita Mensal de Serviços Cloud'} icon={<ComputerDesktopIcon className="w-6 h-6 text-cyan-400" />} className="lg:col-span-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center h-full">
                    <div className="text-center">
                        <p className="text-gray-400 text-base">{'Mensal'}</p>
                        <p className="text-6xl font-bold text-cyan-400">
                            {formatCurrency(financialMetrics.revenueStreamData.find(r => r.name === 'Serviços em Nuvem')?.value || 0)}
                        </p>
                    </div>
            ...
            <DashboardCard title={'Resumo de Lucro Líquido (DRE Simplificado)'} icon={<ChartBarIcon className="w-6 h-6" />} className="lg:col-span-3">
                <div className="h-80 w-full pt-4">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={monthlyNetProfitSummaryData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                            ...
                            <Legend wrapperStyle={{fontSize: "12px", paddingTop: "10px"}}/>
                            <Bar dataKey="revenue" name={'Receita'} fill="#06b6d4" radius={[4, 4, 0, 0]} />
                            <Bar dataKey="totalCosts" name={'Custos Totais + Impostos'} fill="#f59e0b" radius={[4, 4, 0, 0]} />
