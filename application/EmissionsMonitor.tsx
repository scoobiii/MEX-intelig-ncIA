...
import { WarningIcon, InfoIcon, CloseIcon } from './components/icons';
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-gray-700 p-2 border border-gray-600 rounded-md shadow-lg text-sm w-48">
        <p className="font-bold text-gray-300 mb-2">{`Dia: ${label}`}</p>
          ...
          return (
            <div key={config.name} className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-gray-400">{histValue !== undefined ? `(Hist.)` : `(Prev.)`}</span>
              </div>
            </div>
          );
        })}
