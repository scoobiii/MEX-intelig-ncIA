...
const ThermalPlantsSummary: React.FC = () => {
  ...
  return (
    <DashboardCard title="Resumo do Programa de Modernização de Térmicas" icon={<FlameIcon className="w-6 h-6" />}>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 h-full items-center">
        <MetricItem value={totalPlants} label="Usinas no Programa" />
        <MetricItem value={totalPlants} label="Compatíveis com Etanol" />
        <MetricItem value={`${totalPower.toLocaleString('pt-BR')} MW`} label="Potência Total" />
        <MetricItem value={`${totalEthanolDemand.toFixed(1).replace('.',',')} m³/h`} label="Demanda de Etanol" />
